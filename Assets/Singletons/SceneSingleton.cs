...
/// </summary>
/// <typeparam name="T">Type of <seealso cref="SceneSingleton{T}"/></typeparam>
            ...
            lock (_lock)
            {
                if (_instance == null)
                {
                    ...
                    if (_instance == null)
                    {
                        ...
                        LogManager.Log(string.Format( "[Singleton] An instance of {0} is needed in the scene, so '{1}' was created without DontDestroyOnLoad, So it will be destroyed with the scene. To avoid that, use the \"PersistentSingleton\"", typeof(T), singleton));
                    }
                    else
                    {
